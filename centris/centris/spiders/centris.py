...
class Centris(scrapy.Spider):
    ...
    def parse(self,response):
        ...
        result_count = response.xpath('.//*[@class="resultCount"]//text()').get()
    ...
    def listing(self,response):
        ...
        url_list = response.xpath('.//*[@data-summaryurl="SummaryUrl"]/@href').getall()
    ...
    def detail_page(self,response):
        ...
        item = {}
        item['ADDRESS'] = response.xpath('(.//*[@itemprop="address"]/text())[1]').get()
        ...
        if unit_size:
            ...
            for value in unit_size_list:
                ...
                if '7' in value[-1] or '8' in value[-1] or '9' in value[-1] or '10' in value[-1] or 'other' in value[-1].lower():
                    unit_dict['4p_BEDROOM'] =unit_dict['4p_BEDROOM'] + int(value[0])
                elif '1' in value[-1] or '2' in value[-1]:
                    unit_dict['0_BEDROOM'] =unit_dict['0_BEDROOM'] + int(value[0])
                elif '3' in value[-1]:
                    unit_dict['1_BEDROOM'] =unit_dict['1_BEDROOM'] + int(value[0])
                elif '4' in value[-1]:
                    unit_dict['2_BEDROOM'] =unit_dict['2_BEDROOM'] + int(value[0])
                elif '5' in value[-1]:
                    unit_dict['3_BEDROOM'] =unit_dict['3_BEDROOM'] + int(value[0])
                elif '6' in value[-1]:
                    unit_dict['4_BEDROOM'] =unit_dict['4_BEDROOM'] + int(value[0])
        ...
        coordinate_string = response.xpath('.//*[contains(@class,"onmap")]/@onclick').get()
