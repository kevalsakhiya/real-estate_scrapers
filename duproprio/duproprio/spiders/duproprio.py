...
class Duproprio(scrapy.Spider):
    ...
    def detail_page(self, response):
        ...
        price = response.xpath(
            './/*[@class="listing-price__amount"]//text()').get()
        item['PRICE_LISTED'] = self.strToint(price)

        duproprio_number = response.xpath(
            './/*[@class="listing-location__code"]//text()').get()
        item['DUPROPRIO_NUMBER'] = self.strToint(duproprio_number)

        NBL = response.xpath(
            './/*[@class="listing-location__title"]/a/text()').get()
        plex_dictonary = {
            'duplex': '2',
            'triplex': '3',
            'quadruplex': '4',
        ...
        lot_area = response.xpath(
            './/*[contains(@class,"number--dimensions")]/text()').get()
        ...
        features['FLOOR_COVERING'] = self.get_features(
            response, 'Floor coverings:')
        ...
        features['LOT_DESCRIPTION'] = self.get_features(
            response, 'Lot description:')
        ...
        year_assessment = self.get_data(
            response, "Municipal evaluation\'s date")
        ...
        location = {}
        location['latitude'] = response.xpath(
            '.').re_first(r'"latitude":([\d.-]+)')
        location['longitude'] = response.xpath(
            '.').re_first(r'"longitude":([\d.-]+)')
    ...
    def get_data(self, response, keyword):
        ...
        data = response.xpath(
            f'.//*[contains(text(),"{keyword}")]/following-sibling::*[2]/text()').get()
