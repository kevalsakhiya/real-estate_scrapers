...
class RemexQuebec(scrapy.Spider):
    ...
    def listing_page(self,response):
        ...
        url_list = response.xpath('.//a[@class="property-details"]/@href').getall()
        for url in url_list:
    ...
    def detail_page(self,response):
        ...
        address = response.xpath('.//h2[@class="Description__Title"]//text()').getall()
        ...
        item['COLLECTED_DATE'] = datetime.now().strftime(f'%d/%m/%y %H:%M:%S_MLS=>{MLS_NUMBER}')
        ...
        features = {}
        features['Window Type'] = self.property_specification(response,'Window Type')
        features['Windows'] = self.property_specification(response,'Windows')
        features['Foundation'] = self.property_specification(response,'Foundation')
        features['Siding'] = self.property_specification(response,'Siding')
        features['Roofing'] = self.property_specification(response,'Roofing')
        features['Floor Covering'] = self.property_specification(response,'Floor Covering')
        features['Living area'] = self.property_specification(response,'Living area')
        features['Dividing Floor'] = self.property_specification(response,'Dividing Floor')
        features['Lot size'] = self.property_specification(response,'Lot size')
        features['Topography'] = self.property_specification(response,'Topography')
        features['Driveway'] = self.property_specification(response,'Driveway')
        features['Parking'] = self.property_specification(response,'Parking (total)')
        features['Zoning'] = self.property_specification(response,'Zoning')
        features['Heating System'] = self.property_specification(response,'Heating System')
        features['Heating Energy'] = self.property_specification(response,'Heating Energy')
        features['Water Supply'] = self.property_specification(response,'Water Supply')
        features['Sewage System'] = self.property_specification(response,'Sewage System')
        features['Proximity'] = self.property_specification(response,'Proximity')
        ...
        assignment_dictionary['Lot assessment'] = self.finiancial_details(response,'Lot assessment')
        assignment_dictionary['Building assessment'] = self.finiancial_details(response,'Building assessment')
        assignment_dictionary['Municipal assessment'] = self.finiancial_details(response,'Municipal assessment')
        ...
        annual_expences_list = [x.strip() for x in annual_expences_list if x.strip()]
        ...
        energy_expences_list = response.xpath( './/*[contains(text(),"Energy")]/following-sibling::*[1]//*[@class="Financials__Label"]//text()').getall()
        energy_expences_list = [x.strip() for x in energy_expences_list if x.strip()]
