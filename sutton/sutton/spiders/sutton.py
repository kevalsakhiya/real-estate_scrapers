...
class SuttonSpider(scrapy.Spider):
    ...
    def detail_page(self, response):
        ...
        address = response.xpath(
            './/p[contains(text(),"Address")]/following-sibling::p/text()').get()
        item['ADDRESS'] = self.get_clean_data(address)

        price = response.xpath(
            './/*[contains(text(),"Property price")]/following-sibling::*/@value').get()
        item['PRICE_LISTED'] = self.StrToNum(price)

        mls_number = response.xpath(
            './/*[@data-titre="Inscription"]/text()').get()
        ...
        lot_area = response.xpath(
            './/*[contains(text(),"Lot surface")]/following-sibling::td/text()').re_first(r'([\d]+)')
        item['LOT_AREA'] = self.StrToNum(lot_area)

        year_built = response.xpath(
            './/*[@data-titre="Construction"]/text()').get()
        item['YEAR_BUILT'] = self.StrToNum(year_built)

        assessment = {}

        total_assesment = response.xpath(
            './/*[@data-titre="Municipal assessment"]/text()').re_first(r'\$([\d,]+)')
        ...
        income = response.xpath(
            './/*[contains(text(),"Income")]/following-sibling::td/text()').get()
        item['REVENUE'] = {
        ...
        for expense_type in expense_list:
            expense_cost = response.xpath(
                f'.//*[contains(text(),"{expense_type}")]/following-sibling::td/text()').get()
        ...
        municiple_taxes = response.xpath(
            './/*[contains(text(),"Municipal")]/following-sibling::td/text()').get()
        school_taxes = response.xpath(
            './/*[contains(text(),"School taxes")]/following-sibling::td/text()').get()
